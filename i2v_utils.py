...
def load_i2v_model(model_filename, text_encoder_filename, is_720p=False):
    """
    Load the image-to-video model with a specific size config and text encoder.
    """
    if is_720p:
...
def image_to_video(
    ...
    """
    Generate a video from an input image using WAN image-to-video model.
    
    Args:
        image_path (str): Path to the input image
        prompt (str, optional): Text prompt for video generation. Defaults to "".
    ...
    try:
        ...
        if not os.path.isfile(image_path):
            return {
                "status": "error",
                "message": f"Input image does not exist: {image_path}"
            }
        ...
        if output_file is None:
            ...
            output_file = output_dir / f"image2video_{int(time.time())}.mp4"
        
        # Set default model files if not provided
        if transformer_file is None:
            if is_720p:
                transformer_file = f"{DATA_DIR}/i2v-14B-720p.safetensors"
            else:
                transformer_file = f"{DATA_DIR}/i2v-14B-480p.safetensors"
        ...
        # Generate the video
        print(f"Starting image-to-video generation for image: {image_path}")
        sample_frames = wan_model.generate(
            prompt,
            input_img,
            frame_num=frame_count,
            max_area=MAX_AREA_CONFIGS[f"{width}*{height}"],
            shift=flow_shift,
